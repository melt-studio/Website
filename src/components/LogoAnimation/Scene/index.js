...
const Scene = forwardRef(({ fps, name, controls, config, updateConfig, localStorageConfig, updateName, fade }, ref) => {
  ...
  const limit = 2048;
  const targetSize = Math.min(limit, THREE.MathUtils.floorPowerOfTwo(Math.max(size.width, size.height)));
  ...
  useEffect(() => {
    ...
    if (cam.current && size.height < 1000) {
      cam.current.position.z = THREE.MathUtils.mapLinear(size.height, 1000, 0, 90, 50);
  ...
  const clampMouseMovement = () => {
    if (mouse.current.vectorLength < THREE.MathUtils.clamp(1 - mouseArea, 0, 0.99)) {
  ...
  const getFadeTime = () => {
    ...
    if (uFade.z - uFade.w < fd && uTime - uFade.z < fd) {
      ...
      if (uFade.x === 0) {
        let fd0 = ts0 * fd;
        if (uTime < fs) ft = ts0;
        else if (uTime < fs + fd0) ft = THREE.MathUtils.mapLinear(uTime, fs, fs + fd0, ts0, 0);
        else ft = 0;
      } else {
        let fd0 = (1 - ts0) * fd;
        if (uTime < fs) ft = ts0;
        else if (uTime < fs + fd0) ft = THREE.MathUtils.mapLinear(uTime, fs, fs + fd0, ts0, 1);
        else ft = 1;
      }
    } else {
      fe = fs + fd;
