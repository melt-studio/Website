...
vec2 getTexUv(vec2 screenRes, vec2 texRes, float fitWidth) {
  vec2 uv = vUv;
  vec2 uvF = vec2(
    texRes.x / screenRes.x,
    texRes.y / screenRes.y
  );
  uv /= uvF;

  float offside = fitWidth == 1. ? screenRes.x / texRes.x : screenRes.y / texRes.y;

  vec2 uvOff = vec2(
    (screenRes.x - texRes.x * offside),
    (screenRes.y - texRes.y * offside)
  );
  uvOff *= .5;
  uvOff /= texRes.xy;
  uv -= uvOff;
  uv *= fitWidth == 1.? texRes.x / screenRes.x : texRes.y / screenRes.y;

  // float outside = 0.;
  // if (uv.x < 0. || uv.x > 1. || uv.y < 0. || uv.y > 1.) outside = 1.; 

  // return vec3(uv, outside);
  return uv;
}
